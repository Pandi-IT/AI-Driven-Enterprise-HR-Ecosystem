...
public class DataLoader {
    ...
    CommandLineRunner initDatabase(EmployeeRepository empRepo, 
        ...
        return args -> {
            ...
            log.info(">>> 🚀 SEEDING SYSTEM USERS & HR DATA <<<");

            // Seed Admin User (not associated with an employee profile)
            userRepo.save(User.builder()
                    .email("admin@co.com")
                    .password(passwordEncoder.encode("admin123"))
                    .role(Role.ROLE_ADMIN)
                    .build());
            log.info("Seeded Admin User: admin@co.com / admin123");

            // Seed HR User (not associated with an employee profile)
            userRepo.save(User.builder()
                    .email("hr@co.com")
                    .password(passwordEncoder.encode("hr12345"))
                    .role(Role.ROLE_HR)
                    .build());
            log.info("Seeded HR User: hr@co.com / hr12345");
            ...
            userRepo.save(User.builder().email("john.d@co.com").password(passwordEncoder.encode("john123")).role(Role.ROLE_EMPLOYEE).employee(john).build());
            ...
            userRepo.save(User.builder().email("jane.s@co.com").password(passwordEncoder.encode("jane123")).role(Role.ROLE_EMPLOYEE).employee(jane).build());
            attendanceRepo.saveAll(Arrays.asList(
            ...
            userRepo.save(User.builder().email("rob.b@co.com").password(passwordEncoder.encode("rob123")).role(Role.ROLE_EMPLOYEE).employee(rob).build());
            ...
            userRepo.save(User.builder().email("alice.g@co.com").password(passwordEncoder.encode("alice123")).role(Role.ROLE_EMPLOYEE).employee(alice).build());
            ...
            userRepo.save(User.builder().email("bob.w@co.com").password(passwordEncoder.encode("bob123")).role(Role.ROLE_EMPLOYEE).employee(bob).build());
            ...
            userRepo.save(User.builder().email("charlie.b@co.com").password(passwordEncoder.encode("charlie123")).role(Role.ROLE_EMPLOYEE).employee(charlie).build());
            ...
            log.info(">>> 🚀 MASSIVE HR DUMMY DATA & USER CREDENTIALS SEEDED SUCCESSFULLY <<<");
        };
    }
}
